...
describe('mergeHashtags', () => {
  it('dedupes case-insensitively', () => {
    expect(mergeHashtags(['TypeScript', 'nextjs'], ['typescript', 'webdev'])).toEqual([
      'typescript',
      'nextjs',
      'webdev',
    ]);
  ...
  it('strips leading # in both inputs', () => {
    expect(mergeHashtags(['#ts'], ['#nextjs'])).toEqual(['ts', 'nextjs']);
  });
  it('preserves order from the first list', () => {
    expect(mergeHashtags(['b', 'a'], ['c', 'a'])).toEqual(['b', 'a', 'c']);
  });
});
