 ...
 */
type SystemPromptMode = 'draft' | 'edit';

export function buildSystemPrompt(cfg: Config, mode: SystemPromptMode = 'draft'): string {
  ...
  const outputLines =
    mode === 'edit'
      ? [
          ...
          '- This is a chat-edit request. The latest user message contains the exact RESPONSE SHAPE. Follow that wrapper shape.',
          '- For edit intent, the nested `patch` uses the fresh-draft fields: body, content_kind, article?, needs_image, image_source, image_query?, image_concept?, link?, link_placement, hashtags, mentions, pillar, source_url, verbatim_ranges?.',
          '- For reply intent, return only intent and message.',
        ]
      : [
          'Output:',
